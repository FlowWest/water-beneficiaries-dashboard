...
filtered_dataset <- reactive({
  if (input$select_bene == "hydropower") {
    return(all_datasets |> filter(beneficiary_type == "hydropower facility"))
  }
})
...
observe({ 
  req(c(input$select_bene, input$select_metric)) 
  if (input$select_bene == "hydropower") {
    ...
    leafletProxy("map") |> 
      ...
      addPolygons(data = boundary(),
                  popup = ~name) |> 
      addCircleMarkers(data = tmp_data, #filtered_dataset()
                       lng = ~longitude, 
                       lat = ~latitude,
                       opacity = 1, 
                       fillOpacity = 0.8, 
