...
```{r include=FALSE}
url <- "https://services1.arcgis.com/jUJYIo9tSA7EHvfZ/arcgis/rest/services/California_County_Boundaries/FeatureServer/0/query"
...
writeBin(content(res, "raw"), temp_geojson)
county_boundaries <- st_read(temp_geojson)

# filtering out just the counties of interest 
county_boundaries_swp <- county_boundaries |> 
...
county_boundaries_swp <- st_read(shp_path_co) |>
  clean_names() |> 
...
cvp <- cvp_w_locations_2 |> 
  mutate(beneficiary_type = "CVP contractor",
         ...
         quantity_unit = "maximum contract quantity") |> 
...
```{r}
# save clean data
# saveRDS(cvp, here::here("data-raw", "cvp_clean.RDS"))
```
